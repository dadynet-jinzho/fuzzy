...
//        peter.addEdge("created", lop, "weight", 0.2f);
        ...
        //g.V().as("a").select("a").fill(list);
//        g.V().has("name","lop").next().remove();
//        g.V().has("name","ripple").next().property("lang","test");
//        g.V().map(t->t.get().values("name")).fill(list);
        //System.out.println(g.V().map(t->t.get().values("name")).values());
//        g.V().has("name","marko").out("knows").values("name").fill(list);
